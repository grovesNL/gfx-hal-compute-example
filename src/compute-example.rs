...
// thanks to @omni-viral, @termhn, @aleksijuvani on gfx-rs gitter!
...
impl ComputeApplication {
    unsafe fn init() -> ComputeApplication {
        ...
        let (buffer_length, buffer_size, memory, in_buffer, payload, out_buffer, result) =
    ...
    unsafe fn create_io_buffers(
        ...
        *mut u8,
        ...
        let memory = device
            .allocate_memory(memory_type_id, required_size as u64)
            .unwrap();
        ...
        let payload = device
            ...
            .unwrap();
        let result = device
            .map_memory(&memory, (buffer_size as u64)..(required_size as u64))
            .unwrap();
    ...
    unsafe fn set_up_descriptor_sets(
        ...
        let in_descriptor =
            hal::pso::Descriptor::Buffer(in_buffer, Some(0 as u64)..Some(buffer_size as u64));

        let out_descriptor = hal::pso::Descriptor::Buffer(
            out_buffer,
            Some(buffer_size as u64)..Some(2 * buffer_size as u64),
        );
